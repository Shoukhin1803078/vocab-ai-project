...
def toggle_memorized(word):
    ...
    for w in words_list:
        if w['word'] == word:
            ...
            return jsonify({'success': True, 'counts': get_counts(words_list)})
...
.english-meaning {
    margin-bottom: .5rem;  /* Add extra space after English meaning */
    padding-bottom: 0.3rem;
    border-bottom: 1px solid #e2e8f0;  /* Optional: adds a subtle line between meanings */
}
    ...
    <div class="container">
        ...
        <div id="magooshControls" class="controls">
            <button onclick="filterWords('all')" class="filter-button">
                <span>All Words</span>
                <span id="magooshAllCount" class="word-count">0</span>
            </button>
            <button onclick="filterWords('memorized')" class="filter-button">
                <span>Memorized</span>
                <span id="magooshMemorizedCount" class="word-count">0</span>
            </button>
            <button onclick="filterWords('remaining')" class="filter-button">
                <span>Remaining</span>
                <span id="magooshRemainingCount" class="word-count">0</span>
            </button>
        ...
        <div id="myDictControls" class="controls" style="display: none;">
            <button onclick="filterWords('all')" class="filter-button">
                <span>All Words</span>
                <span id="myDictAllCount" class="word-count">0</span>
            </button>
            <button onclick="filterWords('memorized')" class="filter-button">
                <span>Memorized</span>
                <span id="myDictMemorizedCount" class="word-count">0</span>
            </button>
            <button onclick="filterWords('remaining')" class="filter-button">
                <span>Remaining</span>
                <span id="myDictRemainingCount" class="word-count">0</span>
            </button>
    ...
    <script>
        ...
        let currentDictionary = 'magoosh';
            ...
            function switchDictionary(type) {
                ...
                filterWords('all'); // This will update the counts for the selected dictionary
            }
            ...
            async function submitNewWord(event) {
                ...
                try {
                    ...
                    if (response.ok) {
                        ...
                        closeAddWordPopup();
                        filterWords('all'); // This will refresh the display and counts
                    }
                } catch (error) {
        ...
        function updateWordCounts(counts) {
            if (currentDictionary === 'magoosh') {
                document.getElementById('magooshAllCount').textContent = counts.total;
                document.getElementById('magooshMemorizedCount').textContent = counts.memorized;
                document.getElementById('magooshRemainingCount').textContent = counts.remaining;
            } else {
                document.getElementById('myDictAllCount').textContent = counts.total;
                document.getElementById('myDictMemorizedCount').textContent = counts.memorized;
                document.getElementById('myDictRemainingCount').textContent = counts.remaining;
            }
        }
        ...
        function toggleMemorized(event, word) {
                ...
                fetch(`/toggle_memorized/${word}?dictionary=${currentDictionary}`)
                    .then(response => response.json())
                    .then(data => {
                        if (data.success) {
                            filterWords(currentFilter);
        ...
        <div class="card-content">
            ...
            <div class="meanings">
                <div class="meaning english-meaning">
                    <strong>English:</strong> 
                    <span>${word.english_meaning}</span>
                </div>
                
                <div class="meaning native-meaning">
